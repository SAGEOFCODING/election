class ChatAssistant {
    ...
    el.innerHTML = `
      ...
      <div id="chat-panel" style="display:none;flex-direction:column;position:fixed;bottom:90px;right:20px;width:380px;height:520px;background:#fff;border-radius:16px;box-shadow:0 8px 32px rgba(0,0,0,0.15);z-index:1000;overflow:hidden;transition:all 0.2s ease;">
        ...
        <div id="chat-messages" style="flex:1;padding:16px;overflow-y:auto;display:flex;flex-direction:column;gap:12px;background:#f8fafc;">
          <div style="background:#fff;color:#1e293b;border:1px solid #e2e8f0;border-radius:12px 12px 12px 2px;padding:10px 14px;font-size:14px;max-width:85%;align-self:flex-start;line-height:1.5;">Hi! I'm your Election Assistant powered by Claude. Ask me anything about voting.</div>
        </div>
  ...
  async sendMessage() {
    ...
    input.value = '';
    document.getElementById('chips') && (document.getElementById('chips').style.display = 'none');
